...
import { ExternalLink, Eye, Filter, X, CheckCircle2, ArrowRight } from "lucide-react";
  ...
  return (
    <div className="w-full max-w-6xl mx-auto space-y-6 animate-in fade-in duration-500">
          ...
          >
            ...
            <div className="w-full aspect-16/10 bg-slate-100 dark:bg-slate-900 overflow-hidden relative">
              ...
              <button 
                ...
                className="absolute top-3 right-3 p-2 rounded-full bg-white/95 dark:bg-black/60 backdrop-blur-md text-black dark:text-slate-300 hover:text-cyan-600 dark:hover:text-white border border-slate-200 dark:border-white/10 transition-all opacity-0 group-hover:opacity-100 cursor-pointer shadow-xs"
      ...
      {selectedProject && (
        ...
        >
          <div 
            className="glass-panel max-w-2xl w-full rounded-2xl p-6 sm:p-8 space-y-5 border border-slate-200 dark:border-white/20 shadow-2xl relative max-h-[90vh] overflow-y-auto"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="flex items-start justify-between gap-4">
              <div>
                <span className="px-2.5 py-0.5 rounded-full text-xs font-semibold bg-cyan-50 dark:bg-cyan-500/20 text-black dark:text-cyan-300 border border-cyan-200 dark:border-cyan-500/40 mb-2 inline-block">
              ...
              <button 
                onClick={() => setSelectedProject(null)}
                className="p-2 rounded-xl bg-slate-100 hover:bg-slate-200 dark:bg-white/5 dark:hover:bg-white/10 border border-slate-200 dark:border-white/10 text-black dark:text-slate-400 hover:text-cyan-600 dark:hover:text-white transition-colors cursor-pointer"
              >
                <X size={16} />
              </button>
            </div>

            <div className="w-full aspect-video rounded-xl overflow-hidden border border-slate-200 dark:border-white/10">
              <img 
            ...
            {/* Tech Stack Chips */}
            <div>
              <h4 className="text-xs font-bold text-black dark:text-slate-400 uppercase tracking-wider mb-2">Technologies Used</h4>
              <div className="flex flex-wrap gap-2">
                {(selectedProject.tags || []).map((tag, idx) => (
                  <span 
                    key={idx} 
                    className="px-3 py-1 rounded-lg bg-slate-100 dark:bg-white/5 border border-slate-200 dark:border-white/10 text-xs text-black dark:text-cyan-300 font-semibold"
                  >
                    {tag}
                  </span>
                ))}
            ...
            <div className="flex flex-col sm:flex-row gap-3 pt-2">
              {selectedProject.demo && (
                <a 
                  ...
                  target="_blank" 
                  rel="noopener noreferrer"
                  className="flex-1 py-2.5 px-4 rounded-xl bg-linear-to-r from-cyan-600 to-indigo-600 text-white font-medium text-xs sm:text-sm flex items-center justify-center gap-2 shadow-md shadow-cyan-500/20 hover:opacity-95 transition-all"
                >
              ...
              {selectedProject.github && (
                <a 
                  ...
                  target="_blank" 
                  rel="noopener noreferrer"
                  className="flex-1 py-2.5 px-4 rounded-xl bg-slate-100 hover:bg-slate-200 dark:bg-white/5 dark:hover:bg-white/10 border border-slate-200 dark:border-white/10 text-black dark:text-slate-200 font-semibold text-xs sm:text-sm flex items-center justify-center gap-2 transition-all"
                >
                  <FaGithub size={15} />
